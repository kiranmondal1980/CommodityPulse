...
/* Streamlit's built-in icons (sidebar collapse arrow, expander chevrons, etc.)
   are rendered as LIGATURE text — e.g. the literal string
   "keyboard_double_arrow_left" is meant to be substituted for an arrow glyph
   by the Material Symbols font (imported above, alongside the other Google
   Fonts import — @import rules are silently ignored by browsers unless they
   appear before all other CSS, which is why this needs to sit up there and
   not down here). Two things were needed, and the previous pass only did
   the first:
     1. Restore the correct font-family (the broad [class*="css"] rule above
        overrides it with IBM Plex Mono, since Streamlit's auto-generated
        classes also match that selector).
     2. font-feature-settings: 'liga' — ligature substitution is OFF by
        default in browsers. Without this, even the correct font just
        displays the literal characters instead of swapping in the icon. */
